...
select e.employee_id, e.first_name || ' ' || e.last_name as EMPLOYEE_NAME, c.country_name, e.salary
from employees e join departments d on e.department_id = d.department_id
    join locations l on d.location_id = l.location_id
    join countries c on l.country_id = c.country_id;

-- 12. 국가이름, 국가별 급여 합계 검색.
select c.country_name, sum(e.salary) as SUM_SALARY
from employees e join departments d on e.department_id = d.department_id
    join locations l on d.location_id = l.location_id
    join countries c on l.country_id = c.country_id
...
select e.employee_id, e.first_name || ' ' || e.last_name as EMPLOYEE_NAME, j.job_title, e.salary
from employees e join jobs j 
    on e.job_id = j.job_id;

-- 14. 직책이름, 직책별 급여 평균, 최솟값, 최댓값 검색.
select job_title, (min_salary + max_salary) / 2 as AVG_SALARY, min_salary, max_salary
from jobs;

-- 15. 국가이름, 직책이름, 국가별 직책별 급여 평균 검색.
select c.country_name, j.job_title, (j.min_salary + j.max_salary) / 2 as AVG_SALARY
from countries c join locations l on c.country_id = l.country_id
    join departments d on l.location_id = d.location_id
    join employees e on d.department_id = e.department_id
    join jobs j on e.job_id = j.job_id
group by c.country_name, j.job_title, (j.min_salary + j.max_salary) / 2;    

-- 16. 국가이름, 직책이름, 국가별 직책별 급여 합계를 출력.
-- 미국에서, 국가별 직책별 급여 합계가 50,000 이상인 레코드만 출력.
select c.country_name, j.job_title, j.min_salary + j.max_salary as SUM_SALARY
from countries c join locations l on c.country_id = l.country_id
    join departments d on l.location_id = d.location_id
    join employees e on d.department_id = e.department_id
    join jobs j on e.job_id = j.job_id
group by c.country_name, j.job_title, j.min_salary + j.max_salary
having c.country_name = 'United States of America' and j.min_salary + j.max_salary >= 50000;