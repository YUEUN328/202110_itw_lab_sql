-- 제약조건(constraints)
-- 테이블에 데이터들이 insert될 때 조건을 만족하는 값만 insert될 수 있도록 만들어 주는 제약
-- 테이블을 create(생성)할 때 
-- 테이블을 alter(변경)할 때
...
insert into ex01 (ex_id, ex_name, ex, ccol)
values (3, '홍길동', -1);
...
-- insert할 때 다른 테이블 PK에 없는 값을 insert되지 않도록 하기 위해서
...
insert into ex_emp values (100, 'abc', 10);
-- 성공: ex_dept 테이블에 deptno=10인 레코드가 있기 때문에
